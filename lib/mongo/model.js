...
Model.stream = function(query, opts, cb) {
  ...
  if (!cb) {
    return mongo.db.collection(self._collection).find(query, opts).stream({
...
Model.ensured = function(fn) {
  ...
  return function() {
    ...
    if (arg && arg._id && Object.keys(arg).length > 2) {
      arg = args[0] = new cls(arg)
    }
    if (arg instanceof cls || (arg && arg[kind] instanceof cls)) return fn.apply(self, args)

    if (!kind) throw new Error('Ensured class done have kind')

    if (typeof arg === 'string' || typeof arg === 'number') {
      // fn(12346)
      cls.get(arg, function(err, instan) {
        args[0] = instan
        fn.apply(self, args)
      })
    } else {
      // some fn like:
      // fn({ user: xxx })
      cls.get(arg[kind], function(err, instan) {
        args[0][kind] = instan
        fn.apply(self, args)
      })
    }
