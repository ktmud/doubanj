...
function Cached(client, opts) {
  if (!(this instanceof Cached)) return new Cached(client, opts);
  opts = opts || {};
  this.options = opts;
  this.client = client;
  this.constructors = {};
...
Cached.prototype._applykey = function applykey(key, self, args) {
  var ks, v, k;
  return this.options.prefix + key.replace(this._reg_key, function(p0, p1) {
    // dive into object
    ks = p1.split('.');
    v = null;
    k = ks.shift();
...
*/
Cached.prototype.wrap = function(fn, _key, maxAge) {
  var client = this.client;
  var cached = this;
  ...
  return function() {
    ...
    key = cached._applykey(_key, self, args);
    ...
    client.get(key, function(err, reply) {
      ...
      if (reply === undefined) {
        args[args.length -1] = function(err, res) {
          ...
          if (err || res === undefined) return callback.call(self, err, res);
...
Cached.prototype.multi = function(fn, _key, maxAge) {
  var client = this.client;
  var cached = this;
  ...
  return function() {
    ...
    var keys = cached._applykeys(_key, self, args);
